...
use WP_Post;
use PurpleProdigy\ConfigStore as configStore;
...
/**
 * Check if it is ok to save this metabox and it's custom fields.
 *
 * @since 1.0.0
 *
...
function is_okay_to_save_meta_box( $meta_box_key ) {
	...
	// If doing autosave, ajax or future posting, then bail out.
	if (
...
/**
 * Remap the custom fields config parameters, rearrange them
 * into the array structure needed for the saving process.
 *
 * @since 1.0.0
 *
 * @param array $config Custom fields config parameters
 *
 * @return array
 */
function remap_custom_fields_configuration( array $config ) {
...
/**
 * Save custom fields for this meta box.
 *
 * @since 1.0.0
 *
...
function save_custom_fields( $config, $meta_box_key, $post_id ) {
	...
	$config = remap_custom_fields_configuration( $config );
